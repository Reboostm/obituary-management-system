...
export async function sendDirectorFlowerOrderNotification({ directorEmail, deceasedName, customerName, flowerName, serviceDate, serviceTime, serviceLocation }) {
  const html = `
    <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #1f2937;">
      <div style="background: linear-gradient(135deg, #c9a96e 0%, #d4af7f 100%); padding: 20px; text-align: center; border-radius: 8px 8px 0 0;">
        <h1 style="color: white; margin: 0;">💐 New Flower Order</h1>
      </div>
      <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb;">
        ...
        <p style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #d1d5db;">
          <a href="https://didericksenmemorialfuneralservices.com/dashboard" style="background: #d4af7f; color: white; padding: 10px 20px; text-decoration: none; border-radius: 6px; display: inline-block;">View in Dashboard</a>
        </p>
      </div>
    </div>
  `;
  ...
  return sendEmail({
    to: directorEmail,
    subject: `💐 Flower Order for ${deceasedName}`,
    html,
  });
}

export async function sendFloralShopOrderEmail({ floralShopEmail, floralShopName, deceasedName, customerName, customerEmail, customerPhone, flowerName, flowerImage, serviceDate, serviceTime, serviceLocation, deliveryAddress, orderNotes, orderTotal }) {
  const html = `
    <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #1f2937;">
      <div style="background: linear-gradient(135deg, #c9a96e 0%, #d4af7f 100%); padding: 20px; text-align: center; border-radius: 8px 8px 0 0;">
        <h1 style="color: white; margin: 0;">💐 New Flower Order</h1>
      ...
      <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb;">
        ...
        ${orderNotes ? `<h3 style="color: #374151; margin-top: 20px;">Special Instructions</h3>
  ...
  return sendEmail({
    to: floralShopEmail,
    subject: `Flower Order: ${deceasedName} - ${flowerName}`,
    html,
  });
}
