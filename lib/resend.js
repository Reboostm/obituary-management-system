...
function renderServicesBlock(services, fallback) {
  const list = Array.isArray(services) ? services.filter(s => s && (s.date || s.time || s.location || s.type)) : [];
  if (!list.length) {
    const parts = [];
    if (fallback.serviceDate) parts.push(`<p style="margin:8px 0;"><strong>Service Date:</strong> ${fallback.serviceDate}${fallback.serviceTime ? ' at ' + fallback.serviceTime : ''}</p>`);
    if (fallback.serviceLocation) parts.push(`<p style="margin:8px 0;"><strong>Service Location:</strong> ${fallback.serviceLocation}</p>`);
    return parts.join('');
  }
  return list.map(s => {
    const when = [s.date, s.time ? `at ${s.time}` : ''].filter(Boolean).join(' ');
    const where = s.location || '';
    const type = s.type || 'Service';
    return `
      <div style="background:#fffbf2;border-left:4px solid #d4af7f;padding:12px 14px;margin:10px 0;border-radius:6px;">
        <div style="font-weight:700;color:#92400e;text-transform:uppercase;letter-spacing:.05em;font-size:13px;margin-bottom:4px;">${type}</div>
        ${when ? `<div style="color:#1f2937;font-size:14px;margin:2px 0;"><strong>When:</strong> ${when}</div>` : ''}
        ${where ? `<div style="color:#1f2937;font-size:14px;margin:2px 0;"><strong>Where:</strong> ${where}</div>` : ''}
      </div>`;
  }).join('');
}
...
export async function sendFloralShopOrderEmail({
  ...
  const images = (Array.isArray(flowerImages) && flowerImages.length)
    ? flowerImages
    : (flowerImage ? [flowerImage] : []);
  const servicesHtml = renderServicesBlock(services, { serviceDate, serviceTime, serviceLocation });
  ...
  const html = `
    <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #1f2937;">
      ...
      <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb;">
        ...
        <h3 style="color: #374151; margin-top: 20px;">Service &amp; Delivery Schedule</h3>
        <p style="margin:4px 0 10px;color:#6b7280;font-size:13px;font-style:italic;">Please coordinate delivery with the family based on the services below.</p>
        ${servicesHtml}
